...
#python3 run_eval_learnign_old.py
        start = perf_counter()
        ...
        #getting responce in json or dict
        answer = out.get("answer", "") or ""
        docs = out.get("source_documents", []) # settings in memory section



        #saving responce to file
        with open('responce.json','w',encoding='utf-8') as f:
            json.dumps(out,f,ensure_ascii=False,indent=2)

        ids = []

        #docs is list of Document objects
        for d in docs:
            meta = getattr(d, "metadata", {}) or {}
            id_str = str(meta.get("ids", ""))
            ids.append(id_str)
